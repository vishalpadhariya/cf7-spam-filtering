...
class CF_Spam_Filtering
{
    ...
    public function save_cf7_spam_filtering_form_meta($contact_form)
    {
        ...
        $spam_domains = isset($_POST['cf7_spam_domains']) ? sanitize_textarea_field($_POST['cf7_spam_domains']) : '';
     ...
     */
    public function spam_filtering_tab_content($post)
    {
        $html  = '<div class="cf7-spam-filtering-options">';
        $html .= '<h3>' . _e('Spam Filtering Options', 'cf7-spam-filtering') . '</h3>';
        $html .= '<p>' . _e('Add domains to prevent in form submissions:', 'cf7-spam-filtering') . '</p>';
        $html .= '<textarea name="cf7_spam_domains" rows="5" cols="50">' . esc_textarea(get_post_meta($post->id(), '_cf7_spam_domains', true)) . '</textarea>';
        $html .= '</div>';
    ...
    public function validate_email($result, $tag)
    {
        ...
        $email_field_name = $this->get_cf7_email_field_name($form_id);

        if ('' !== $email_field_name) {
            $email = isset($_POST[$email_field_name]) ? sanitize_email(trim($_POST[$email_field_name])) : '';
        } else {
    ...
    public function get_cf7_email_field_name($form_id)
    {
        ...
        $email_object = array_filter($form_fields, function ($obj) {
